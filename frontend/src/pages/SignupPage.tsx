  ...
  return (
    <div className="max-w-sm mx-auto bg-white p-6 rounded shadow-sm">
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <input className="w-full border rounded px-3 py-2" placeholder="비밀번호 (6자 이상)" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
        ...
        <button className="w-full bg-slate-800 text-white rounded py-2" type="submit">가입</button>
      </form>
      <div className="text-sm text-slate-500 mt-3">이미 있으시면 <Link to="/login" className="text-slate-800 underline">로그인</Link></div>
    </div>
  );
}
